...
def api_report_pdf(
    ...
    require_api_key(x_api_key, x_forwarded_for)
    try:
        # 1) generate text using the same prompt
        summary = summarise_transactions()
        ...
        persist("/api/report_pdf", summary, report, x_forwarded_for, x_api_key)

        # 2) write a PDF
        ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        path = os.path.join("exports", f"report_{ts}.pdf")
        ...
        c.drawString(20*mm, y, "Project Z — Daily Business Report")
        y -= 8*mm
        c.setFont("Helvetica", 10)
        c.drawString(20*mm, y, f"Date: {today}")
        y -= 8*mm
        c.drawString(20*mm, y, f"Summary: {summary}")
        y -= 10*mm
        ...
        def draw_wrapped(text, y_pos):
            ...
            for line in text.splitlines():
                lines = wrap(line, 95)  # rough width for A4/Helvetica 11
                for wline in lines:
                    if y_pos < 20*mm:
                        c.showPage()
                        y_pos = height - 20*mm
                        c.setFont("Helvetica", 11)
                    c.drawString(20*mm, y_pos, wline)
                    y_pos -= 6*mm
            return y_pos

        y = draw_wrapped(report, y)
        c.showPage()
        c.save()
